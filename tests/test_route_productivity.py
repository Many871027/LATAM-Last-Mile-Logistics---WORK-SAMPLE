...
class TestRouteProductivity(unittest.TestCase):
    def setUp(self):
    ...
    def test_metrics_calculation_and_report_generation(self):
        print("\n[TEST] Querying country-level productivity metrics from BigQuery...")
        ...
        # Verify Colombia stops efficiency: 90.53%, capacity utilization: 78.85%
        co_row = country_summary[country_summary['country'] == 'CO'].iloc[0]
        self.assertAlmostEqual(co_row['stops_efficiency_pct'], 90.53, delta=1.5)
        self.assertAlmostEqual(co_row['capacity_utilization_pct'], 78.85, delta=1.5)

        # Verify Brazil stops efficiency: 90.33%, capacity utilization: 65.00%
        br_row = country_summary[country_summary['country'] == 'BR'].iloc[0]
        self.assertAlmostEqual(br_row['stops_efficiency_pct'], 90.33, delta=1.5)
        self.assertAlmostEqual(br_row['capacity_utilization_pct'], 65.00, delta=1.5)

        # 2. Query underperforming routes (stops efficiency < 60%)
        print("[TEST] Querying underperforming routes summary from BigQuery...")
        ...
        """
        underperforming_summary = self.client.query(underperforming_query).to_dataframe()

        # Expected underperforming counts: CO: 63, PE: 21, AR: 29, MX: 79, BR: 38, CL: 14
        ...
        for country, count in expected_underperforming.items():
            actual_count = underperforming_summary[underperforming_summary['country'] == country]['underperforming_routes_count'].iloc[0]
            self.assertAlmostEqual(actual_count, count, delta=2, msg=f"Mismatch for {country}: expected {count}, got {actual_count}")

        # 3. Query worst 5 performing routes in Brazil
        print("[TEST] Querying top 5 worst performing routes in Brazil from BigQuery...")
        worst_5_query = """
        WITH deduped_shipments AS (
          SELECT 
            shipment_id, 
        ...
        LIMIT 5;
        """
        worst_5 = self.client.query(worst_5_query).to_dataframe()

        # 4. Generate markdown report based on cloud results
        print("[TEST] Writing Question_2_Route_Productivity_Report.md...")
        report_path = "D:/wt-harness-MELI_BI/Reports/Question_2_Route_Productivity_Report.md"
