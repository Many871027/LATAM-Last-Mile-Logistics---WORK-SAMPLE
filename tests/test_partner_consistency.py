...
class TestPartnerConsistency(unittest.TestCase):
    def setUp(self):
        ...
        try:
            ...
            self.fail(f"Failed to initialize BigQuery client: {str(e)}")

    def test_partner_consistency_and_report_generation(self):
        print("\n[TEST] Auditing partner PT-014 consistency metrics from BigQuery...")
        ...
        """
        stale_df = self.client.query(stale_query).to_dataframe()
        self.assertFalse(stale_df.empty, "Stale routes query returned no data for PT-014")
        ...
        GROUP BY pr.partner_id;
        """
        gps_chrono_df = self.client.query(gps_chrono_query).to_dataframe()
        self.assertFalse(gps_chrono_df.empty, "GPS and chronology query returned no data for PT-014")
        ...
        """
        overlap_df = self.client.query(overlap_query).to_dataframe()
        if overlap_df.empty:
            overlap_row = {
                'partner_id': 'PT-014',
        ...
        """
        contract_df = self.client.query(contract_query).to_dataframe()
        self.assertFalse(contract_df.empty, "Contract query returned no data for PT-014")
        contract_row = contract_df.iloc[0]

        # 5. Unified audit query
        unified_query = """
        WITH partner_routes AS (
          SELECT
            r.route_id,
        ...
        """
        unified_df = self.client.query(unified_query).to_dataframe()
        self.assertFalse(unified_df.empty, "Unified audit query returned no data for PT-014")
        unified_row = unified_df.iloc[0]

        # Basic validations
        self.assertEqual(unified_row['partner_id'], 'PT-014')
        self.assertEqual(unified_row['partner_name'], 'SaoPauloShip')
        self.assertGreater(unified_row['total_routes_assigned'], 0)

        # Confirm contract expired percentage is 100% since active_flag = 0 and routes are in 2025
        self.assertAlmostEqual(unified_row['contract_expired_routes_pct'], 100.00, delta=0.1)

        # Confirm that error rate exceeds 5% (Requirement A8)
        # Combined rate of chrono violations, stale routes, and overlapping routes:
        ...
        print(f"\n[TEST] Combined Error Count: {combined_error_count} / {total_routes} ({combined_error_pct:.2f}%)")
        self.assertGreater(combined_error_pct, 5.0, "Combined error rate must exceed 5% exclusion threshold")

        # Generate report
        base_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        report_path = os.path.join(base_dir, "Reports", "Question_6_Partner_Consistency_Report.md")
...
Debido a que el porcentaje combinado de errores operativos y de registro es de **{combined_error_pct:.2f}%** (excediendo con creces el límite del **5%** establecido por la regla **A8**), se emite la recomendación definitiva de **EXCLUIR a PT-014 (SaoPauloShip) de los reportes analíticos oficiales**.
...
"""
        ...
        print(f"[TEST] Report Question_6_Partner_Consistency_Report.md generated successfully at {report_path}.")
